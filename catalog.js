"use strict";
class Good {
    constructor({ id_product: id, product_name: title, price, img, description }) {
        this._id = id;
        this._img = img;
        this._title = title;
...
const list = new GoodsList(document.querySelector('.card__box'));
console.log(list);
fetch('https://raw.githubusercontent.com/GeekBrainsTutorial/online-store-api/master/responses/catalogData.json')//как создать JSON????, на уроке вы сказали , что для макета это сложно, создавайте сами, так хоть объясните КАК!!!
    .then((Response) => {
